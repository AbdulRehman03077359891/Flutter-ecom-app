...
class NOrderPending extends StatefulWidget {
  ...
  @override
  State<NOrderPending> createState() => _NOrderPendingState();
}

class _NOrderPendingState extends State<NOrderPending> {
  ...
  Widget build(BuildContext context) {
    return GetBuilder<NormUserController>(builder: (normUserController) {
      return Scaffold(
        ...
        body: normUserController.pendingOrders.isEmpty
            ? SizedBox(
            ...
            ListView.builder(
                ...
                shrinkWrap: true,
                itemCount: normUserController.pendingOrders.length,
                    ...
                    child: Stack(
                      children: [
                        ListTile(
                          leading: GestureDetector(
                            onTap: () {
                              animateController.showSecondPage("pic$index", 
                                            normUserController.pendingOrders[index]
                                                ["imageUrl"], context);
                            },
                            child: Card(
                              elevation: 10,
                          ...
                          title: Text(
                            normUserController.pendingOrders[index]["name"],
                            style: const TextStyle(fontWeight: FontWeight.w500),
                          ),
                          subtitle: Column(
                            children: [
                              Text(
                                  "Quantity: ${normUserController.pendingOrders[index]['quantity'].toString()}"),
                              Text(
                                "Per Item Rs ${(int.parse(normUserController.pendingOrders[index]["price"]) / normUserController.pendingOrders[index]["quantity"]).toStringAsFixed(2)}",
                                softWrap: true,
                                maxLines: 2,
                              ),
                            ],
                            ...
                            child: Stack(
                              children: [
                                Positioned(
                                    ...
                                    right: 0,
                                    child: Text(
                                        "Rs ${normUserController.pendingOrders[index]['price']}")),
                                Positioned(
                                    bottom: 20,
                                    right: 0,
                                    child: Text(
