...
class BOrderDilivered extends StatefulWidget {
  final String userUid, userName, userEmail, status;
  const BOrderDilivered({
    super.key,
  ...
  @override
  State<BOrderDilivered> createState() => _BOrderDiliveredState();
}

class _BOrderDiliveredState extends State<BOrderDilivered> {
  ...
  Widget build(BuildContext context) {
    return GetBuilder<BusinessController>(builder: (businessController) {
      return Scaffold(
        ...
        body: businessController.deliveredOrders.isEmpty
            ? SizedBox(
            ...
            SingleChildScrollView(
                child: ListView.builder(
                    ...
                    shrinkWrap: true,
                    itemCount: businessController.deliveredOrders.length,
                    itemBuilder: (context, index) {
                      return Card(
                        ...
                        child: ListTile(
                          ...
                          title: Text(
                            businessController.deliveredOrders[index]["name"],
                            style: const TextStyle(fontWeight: FontWeight.w500),
                          ),
                          subtitle: Column(
                            children: [
                              Text(
                                  "Quantity: ${businessController.deliveredOrders[index]['quantity'].toString()}"),
                              Text(
                                "Per Item Rs ${(int.parse(businessController.deliveredOrders[index]["price"]) / businessController.deliveredOrders[index]["quantity"]).toStringAsFixed(2)}",
                                softWrap: true,
                                maxLines: 2,
                              ),
                            ],
                            ...
                            child: Stack(
                              children: [
                                Positioned(
                                    ...
                                    right: 0,
                                    child: Text(
                                        "Rs ${businessController.deliveredOrders[index]['price']}")),
                                Positioned(
                                    bottom: 20,
                                    right: 0,
                                    child: Text(businessController
                                        .deliveredOrders[index]["status"],style: const TextStyle(fontWeight: FontWeight.bold),)),
                              ],
                            ),
                          ),
                        ),
